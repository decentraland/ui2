...
const StyledTableBody = styled(TableBody, {
  ...
  ['& tr td:first-of-type:not(:only-of-type),' + ' & tr th:first-of-type:not(:only-of-type)']: {
    borderRadius: theme.spacing(2, 0, 0, 2)
  },
  ['& tr td:last-of-type:not(:only-of-type),' + ' & tr th:last-of-type:not(:only-of-type)']: {
    borderRadius: theme.spacing(0, 2, 2, 0)
  },
  '& tr': {
    borderRadius: theme.spacing(2),
...
const isResponsiveWidth = (value: ResponsiveWidth): value is { mobile?: string | number; desktop?: string | number } =>
  typeof value === 'object' && ('mobile' in value || 'desktop' in value)

const StyledTableCell = styled(TableCell)<StyledTableCellProps>(({ theme, $cellWidth, $cellPadding, $hasBorder }) => {
  const desktopWidth = $cellWidth && isResponsiveWidth($cellWidth) ? $cellWidth.desktop : $cellWidth
  const mobileWidth = $cellWidth && isResponsiveWidth($cellWidth) ? $cellWidth.mobile : undefined
...
const StyledTableRow = styled(TableRow, {
  shouldForwardProp: prop => prop !== 'mobileClickable' && prop !== 'withBorder'
...
const TableCellBorderContainer = styled(Box)<TableCellBorderContainerProps>(({ theme, $borderColor, $width }) => ({
  position: 'absolute',
  top: 0,
  left: 0,
  width: $width || '100%',
