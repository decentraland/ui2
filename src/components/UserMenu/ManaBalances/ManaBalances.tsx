...
const ManaBalances = React.memo((props: ManaBalancesProps) => {
  ...
  const creditsExpiringSoon = creditsBalance && creditsBalance.expiresAt < Date.now() + 1000 * 60 * 60 * 24 * 5 // 5 days
  ...
  const creditsExpiringIn = creditsBalance
    ? Math.max(0, Math.floor((creditsBalance.expiresAt - Date.now()) / (1000 * 60 * 60 * 24)))
    : undefined
      ...
      <Box>
        {manaBalances &&
          Object.keys(manaBalances).map(network => (
            ...
            >
              <ManaBalanceNumber variant="subtitle2">{Number(manaBalances[network].toFixed(2)).toLocaleString()}</ManaBalanceNumber>
            </Mana>
          ))}
      </Box>
        ...
        <CreditsBalanceContainer>
          <>
            {creditsBalance.balance === 0 ? (
              ...
              <>
                {creditsExpiringSoon && <ExperingSoon>{i18n?.creditsExpiringSoon}</ExperingSoon>}
                {creditsExpiringIn !== undefined && (
                  <Tooltip
                    title={
                      <CreditsBalanceTooltipContainer>
                        <span>{i18n?.creditsExpiringIn(creditsExpiringIn === 0 ? '< 1' : creditsExpiringIn.toString())}</span>
