...
import { Box as BoxMui, Modal as MuiModal, Paper as PaperMui } from '@mui/material'
import { ModalProps } from './Modal.types'

interface StyledModalProps extends Pick<
  ModalProps,
  'size' | 'backgroundImage' | 'backgroundSize' | 'backgroundPosition' | 'backgroundRepeat'
> {}
...
const ModalContainer = styled(PaperMui)<StyledModalProps>(props => {
  const { size, theme, backgroundImage, backgroundSize, backgroundPosition, backgroundRepeat } = props
  let width = '900px'
  if (size === 'tiny') {
    width = '540px'
  } else if (size === 'small') {
...
export { Modal, ModalContainer, ModalTitleContainer, ModalContentContainer, ModalActionsContainer }
