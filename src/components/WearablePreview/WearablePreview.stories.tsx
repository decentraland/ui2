  ...
  render: () => {
    ...
    return (
      <div style={{ padding: '20px' }}>
        ...
        >
          <button onClick={() => ref.current?.scene.getScreenshot(1024, 1024).then(setScreenshot)}>Take Screenshot</button>
          <button onClick={() => ref.current?.scene.getMetrics().then(setMetrics)}>Get Metrics</button>
        </div>
        {screenshot && (
          <div style={{ textAlign: 'center', marginTop: '20px' }}>
            <img src={screenshot} alt="Screenshot" style={{ maxWidth: '256px' }} />
          </div>
        )}
        {metrics && (
          <div
  ...
  render: () => {
    ...
    return (
      <div style={{ padding: '20px' }}>
        {file ? (
          <WearablePreviewContainer>
            <WearablePreview id="some-id" profile="default" blob={toWearableWithBlobs(file)} />
            <AnimationControls wearablePreviewId="some-id" />
          </WearablePreviewContainer>
        ) : (
          <div style={{ textAlign: 'center', padding: '50px' }}>
            <input type="file" ref={inputRef} accept=".glb,.gltf" onChange={() => setFile(inputRef.current?.files?.[0] || null)} />
...
export const UnityModes: Story = {
  ...
  render: args => {
    ...
    const config = modeConfigs[args.unityMode as keyof typeof modeConfigs] || modeConfigs.marketplace
...
export const ConfiguratorWithUsernameControl: Story = {
  render: () => {
    ...
    const ref = useRef<ReturnType<typeof WearablePreview.createController> | null>(null)
    ...
    return (
      <div style={{ padding: '20px' }}>
        <WearablePreviewContainer>
          <WearablePreview id="configurator-id" unity={true} unityMode={PreviewUnityMode.CONFIG} />
        </WearablePreviewContainer>
        <div
          style={{
            display: 'flex',
...
export const WithZoomControls: Story = {
  render: () => {
    ...
    return (
      <WearablePreviewContainer>
        <WearablePreview id={previewId} profile={getRandomProfile()} disableAutoRotate zoom={100} wheelZoom={2} />
...
export const WithTranslationControls: Story = {
  render: () => {
    ...
    return (
      <WearablePreviewContainer>
        <WearablePreview id={previewId} profile={getRandomProfile()} disableAutoRotate />
        <TranslationControls wearablePreviewId={previewId} vertical verticalPosition={VerticalPosition.RIGHT} />
      </WearablePreviewContainer>
    )
  }
}
    ...
    return (
      <WearablePreviewContainer>
        <WearablePreview
          ...
          onLoad={handleLoad}
        />
        <EmoteControls wearablePreviewId={previewId} wearablePreviewController={ref.current ?? undefined} />
        <ZoomControls wearablePreviewId={previewId} position={Position.LEFT} wearablePreviewController={ref.current ?? undefined} />
        <TranslationControls
