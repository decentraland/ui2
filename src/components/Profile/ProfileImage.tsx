...
const ProfileImage = memo(<T extends React.ElementType>(props: ProfileProps<T>) => {
  // It's not a good practice to default inline to true, but maybe we should keep it for backwards compatibility
  const { isDecentraland, avatar, inline = true, address, size = 'normal', rounded = false, as, ...rest } = props
  ...
  if (avatar) {
    return <AvatarFace size={avatarFaceSize} inline={inline} avatar={avatar} />
  }
