...
const Blockie = React.memo((props: BlockieProps) => {
  const { seed, color = '#e449c2', spotcolor = '#e449c2', bgcolor = '#3149de', size = 6, scale = 6, children, rounded = false } = props
  ...
  return children ? (
    <Box component="span">
      {canvas}
      <StyledBlockieChildrenWrapper component="span">{children}</StyledBlockieChildrenWrapper>
    </Box>
  ) : (
    canvas
  )
