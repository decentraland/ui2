...
import { rarity as rarityColors, rarityLightTheme, textOnRarity } from '../../theme/colors'
...
const RarityBadge = styled(Chip, {
  ...
  const rarityBackground =
    palette.rarities?.[rarity] ?? (palette.mode === 'dark' ? hexToRgba(rarityColors[rarity], 0.2) : rarityLightTheme[rarity])
