...
const ScenesTable = memo((props: ScenesTableProps) => {
    ...
    () => [
      {
        ...
        cellPadding: 0,
        render: row => <SceneRow name={row.sceneName} thumbnail={row.thumbnail} creator={row.creator} location={row.location} />
      },
      {
        id: 'creator',
        header: 'Creator',
      ...
      },
      {
        ...
        hideOnMobile: true,
        render: row => (
          <ActionRow positionChange={row.positionChange} location={row.location} onJumpInTrack={data => onJumpInTrack?.(data, row)} />
        )
      }
    ],
    [onJumpInTrack]
  ...
  return <Table columns={columns} rows={rows} headerVisible={headerVisible} onMobileRowClick={onMobileRowClick} />
})
