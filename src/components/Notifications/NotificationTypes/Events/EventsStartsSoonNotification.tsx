...
const EventsStartsSoonNotification = React.memo((props: CommonNotificationProps<EventsStartsSoonNotificationProps>) => {
      ...
      description={
        new Date(notification.metadata.startsAt).getTime() > Date.now() ? (
          <>
            ...
            <Link href={notification.metadata.link}>{notification.metadata.name}</Link>{' '}
            {eventStartsSoonI18n[locale].description.aboutToStart.end} <Countdown startDate={notification.metadata.startsAt} />
          </>
        ) : (
          <>
