...
const GiftNotification = React.memo((props: CommonNotificationProps<GiftNotificationProps>) => {
  const { notification, locale, renderProfile } = props
  const player = renderProfile?.(notification.metadata.senderAddress) ?? <Address shorten value={notification.metadata.senderAddress} />
