...
const TipNotification = React.memo((props: CommonNotificationProps<TipNotificationProps>) => {
  const { notification, locale, renderProfile } = props

  const senderAddress = notification.metadata.senderAddress
  const playerName = renderProfile?.(senderAddress) ?? `${senderAddress.slice(0, 6)}…${senderAddress.slice(-4)}`
  const manaAmount = Number(notification.metadata.amount)
    .toFixed(2)
    .replace(/\.?0+$/, '')
  ...
  const playerLink = (
    <Link href={profileUrl} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}>
      {playerName}
    </Link>
  )

  const manaAmountLink = (
    <Link href={marketplaceActivityUrl} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}>
      {manaAmount} MANA
    </Link>
  )
