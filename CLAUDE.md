...
- **Per-scheme values belong in `src/theme/colorSchemes.ts`, never in a component folder.** Component-scoped ones go under `palette._components.<component>`, next to the existing `appBar` / `tooltip` / `chip` entries, and the component reads them off the theme inside `styled()` — `theme.palette._components.navbar`, the same way `RarityBadge.styled.ts:20` reads `theme.palette.rarities`. Custom palette keys survive `createPalette` → `extendTheme` → `CssVarsProvider`, so this works in both themes. Declare the type in `src/types/emotion.ts`. Heads-up: `_components` had no runtime reader before the navbar (`components.ts` never reads it), so the only worked examples are the top-level keys.
- **Do not build a parallel `const FOO_SCHEMES = { light: …, dark: … }` map inside `src/components/<Name>/`.** It duplicates the token system and puts design values where no designer or consumer will look for them. Caught in review on PR #472.
- A one-off inline `theme.palette.mode === 'dark' ? A : B` is a different thing and is **not** covered by that ban: it is the incumbent pattern in `EventCard`, `EventSmallCard`, `SceneCard` and `CatalogCard` (14 sites). Prefer palette tokens for anything that is a set of values, but do not migrate those four drive-by; consolidating them is its own decision.
- `src/theme/components.ts` is **not** the alternative: it emits MUI `styleOverrides` keyed by component name and covers six (`MuiCssBaseline`, `MuiAppBar`, `MuiButton`, `MuiPaper`, `MuiTabs`, `MuiTab`), so it cannot reach a custom component built with `@emotion/styled` on plain `nav` / `button` / `a` elements. It holds style _rules_ and reads its _values_ off the palette — keep that split.
- To branch on the active scheme (only needed in `components.ts`), use `palette.mode`. `palette.colorScheme` is unreliable: it stays `light` for both themes, because `theme('dark')` passes a flat palette that `extendTheme` files under `colorSchemes.light`.
